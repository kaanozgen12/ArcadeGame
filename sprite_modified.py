...
class AnimatedWalkingSprite(Sprite):
    ...
    def __init__(self, scale: float = 1,
        ...
        self.jump_up_textures: List[Texture] = []
        self.jump_down_textures: List[Texture] = []
        self.cur_texture_index = 0
    ...
    def update_animation(self, delta_time: float = 1/60):
        ...
        if self.change_x >= 0 \
            ...
            change_direction = True
                ...
                and len(self.walk_left_textures) > 0 and not self.state == FACE_UP and not  self.state == FACE_DOWN :
            self.state = FACE_LEFT
            change_direction = True
        elif self.change_y >0 and self.change_x<0 and  self.state != FACE_DOWN:
            self.state = FACE_DOWN
            change_direction = True
        elif self.change_y > 0 and self.state!=FACE_DOWN and self.state != FACE_UP  \
                and len(self.walk_up_textures) > 0:
            self.state = FACE_UP
            change_direction = True
        elif self.change_y < 0 and not self.state == FACE_UP and not  self.state == FACE_DOWN  and self.change_x == 0 and len(self.walk_up_textures) > 0:
            self.state = FACE_RIGHT
